<<
  %% Chœur (dessus)
  \tag #'(choeur basse) {
    << { s1 s2.*2 s1 s2. s1 s2. s1 s1 s1*6 s2. s1*4 s2. s1 s2. s1 s1 s1*2 s1 }
      \tag #'choeur { \clef "vbas-dessus" R1 R2.*2 R1 R2. R1 R2. R1 R1 R1*6
        R2. R1*4 R2. R1 R2. R1 R1 R1*2 R1 \break } >>
    ...
    << { s1*2 s2.*2 s1 s2. s1 s2.*3 s1*12 s2. s1*3 s2. s1*4 s2. }
      \tag #'choeur { R1*2 | R2.*2 | R1 | R2. | R1 |
        R2.*3 | R1*12 | R2. | R1*3 | R2. | R1*4 | \break r2 r4 } >>
  ...
  \tag #'(cybele basse) {
    ...
    do''4
    << { s2 s1*8 s2. s4 } \tag #'cybele { r2 | R1*8 | R2. | r4 } >>
    \tag #'basse \cybeleMark r16 la'-\tag #'cybele ^\markup\italic {
      [montrant à Atys Sangaride morte]
    } la' la' fad'4 r8 re'' |
    si'\trill si' r4
    << { s2 s1*2 s2. s4 } \tag #'cybele { r2 | R1*2 | R2. | r4 } >>
    \tag #'basse \cybeleMark r8 mi'' la'4 la'8 la'16 la' |
    fad'4 la'8 la'16 la' la'8. si'16 |
    sol'4
    << { s2. s1*13 s2.*2 s1 s2. s1 s2.*3 s4. }
      \tag #'cybele { r4 r2 | R1*13 | R2.*2 | R1 | R2. | R1 | R2.*3 | r4 r8 } >>
    \tag #'basse \cybeleMark mib''8
    -\tag #'cybele ^\markup\mod-version\character Cybele
    si'8 si'16 si' si'8 si'16 re'' |
  ...
  \tag #'(atys basse) {
    ...
    << \tag #'atys sol'4 \tag #'basse { sol'8 s } >>
    << { s2.*2 s1 s4 } \tag #'atys { r4 r2 | R2. | R1 | r4 } >>
    \tag #'basse \atysMark r8 sol sol8. sol16 |
    ...
    fa'4 r16 re' re' re' \myfootnote #'NoteHead #'(0 . 4) \markup {
    ...
    \myfootnote #'NoteHead #'(0 . 4) \markup {
      ...
      (les deux derniers \italic si raturés et corrigés en
      \concat { \italic sol ) }
      \hspace #2
      Manuscrit : \raise #3 \score {
        <<
    ...
    dod'4
    << { s2.*2 s4 } \tag #'atys { r4 r2 | R2. | r4 } >>
    \tag #'basse \atysMark re'4 r8 re' si\trill si16 si |
    do'4 do'8 do' sol4 la8 sib |
    la\trill la r la' fa'8. fa'16 do'8. re'16 |
    mi'8 mi'16 do' do' re' mi' fa' sol'8 sol'16 sol' do'8 do'16 re' |
    si2\trill r |
    << { s1*7 } \tag #'atys { R1*7 } >>
    \tag #'basse \atysMark r4-\tag #'atys ^\markup\mod-version\character Atys
    mib' do'8. do'16 do'8. sol16 |
    ...
    \myfootnote #'NoteHead #'(0 . 1.5) \markup {
      Manuscrit : \raise #3 \score {
        <<
          { \tinyQuote \set autoBeaming = ##f
            \key sib \major \clef "alto" \time 4/4
            mib'8. mib'16 mib' la la sib sib8 sib r re'16 re' | mid'8.
          } \addlyrics {
    ...
    \tag #'basse \atysMark re'4-\tag #'atys ^\markup\mod-version\character Atys
    re'16 re' re' re' |
