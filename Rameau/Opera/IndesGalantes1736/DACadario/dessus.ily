\clef "dessus"
<<
  \tag #'violon1 {
    la''8 |
    la''4.( sol''8) sol''8( fa''16\trill mi'') |
    \appoggiatura mi''8 fa''8.[ mi''16]
  }
  \tag #'violon2 {
    fa''8 |
    fa''4. mi''8 mi''8. mi''16 |
    la'4
  }
>> re''8.[( do''16)] sib'8.[( la'16)] |
sol'2.~ |
sol'4.( fa'16\trill mi') \appoggiatura mi'8 fa'8. la'16 |
re'4.( mi'16 fa') mi'8.(\trill re'16) |
dod'8.\trill( re'16) mi'2 |
la4 re'2~ |
re' sib'8.( la'16) |
la'2( sol'8.)\trill sib'16 |
\appoggiatura fa'8 mi'2\trill r8 <<
  \tag #'violon1 {
    la''8 |
    la''4.( sol''8) sol''8( fa''16\trill mi'') |
    \appoggiatura mi''8 fa''8.( mi''16)
  }
  \tag #'violon2 { dod''8 | re''2 mi''4 | la' }
>> re''8.( do''16) sib'8.( la'16) |
sol'2.~ |
sol'4.( fa'16\trill mi') \appoggiatura mi'8 fa'8.( la'16) |
re'4.( mi'16 fa') mi'8.\trill( re'16) |
dod'8 \appoggiatura si la16 r r4 mi'8.( re'16) |
fa'8.( mi'16) sol'4 sib' |
mi'8.( sol'16) fa'8.\trill( mi'16) la'8.( fa'16) |
\appoggiatura mi'8 re'2~ re'8 dod'16( re') |
re'4( dod'4.\trill) re'8 |
re'2 r8 <<
  \tag #'violon1 {
    la'8
    sib'8.( sol'16) si'4.( la'16\trill sol') |
    do''8.( la'16) dod''4.( si'16\trill la') |
    re''8. fa'16 mi'4.\trill re'8 |
    re'4\fermata
  }
  \tag #'violon2 {
    la'8
    re'4 sol'2~ |
    sol'8. sol'16 mi'4\trill la'~ |
    la'8. re'16 dod'4.\trill re'8 |
    re'4\fermata
  }
>> r4 r |
fa'4 mi'8.\trill( re'16) mib'8.( fa'16) |
re'8\trill sol'4. fa'8 mi' |
la4.( si8) do'8.( re'16) |
si4\trill la sold\trill |
la fa'4. mi'16( la') |
la'2~ la'8 la'16 sold' |
\appoggiatura sold'8 la'2 r8 <<
  \tag #'violon1 la''8
  \tag #'violon2 dod''
>>

