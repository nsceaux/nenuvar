<<
  ...
  \tag #'(damon basse) {
    <<
      \tag #'damon {
        \clef "vhaute-contre" R1*3 R2.*3 R1 R2.*4 r4 r8 r8 
        <>^\markup\character Damon
      }
      \tag #'basse { s1*3 s2.*3 s1 s2.*4 s4. \damonMark r8 }
    >>
    la8 si |
    ...
    fa4 la16 si do'8 si8.\trill dod'16 |
    \appoggiatura dod'8 re'4 mi'16 fa' \appoggiatura fa'8 mi'\trill sol'16[ fa'] mi'[ re'] |
    dod'4 \appoggiatura sib8 la8 mi' fa'16 mi' dod' re' |
    sol'4. la'8 fa'4.\trill mi'8 mi'4.\trill re'8 |
    re'4 
    re'1 |
    r4^"Recitatif" la8 la sib4 sib8 sib |
    fad4\trill re'8 re' la4\trill la8 sib |
    \appoggiatura la8 sol sol sib16 sib sib do' re'8 mi'16 fa' |
  ...
  \tag #'(alvar basse) {
    \clef "vbasse" R1*3 |
    r4 r r8 <>^\markup\character Alvar la |
    ...
    <<
      \tag #'alvar {
        r4 r8 | R2.*9 R8*9 R2.*8 R1. r4 R1*3 R2.*2 |
        r4 <>^\markup\italic { [apercevant Zima] }
      }
      \tag #'basse {
        s4. s2.*9 s8*9 s2.*8 s1. s4 s1*3 s2.*2 s4
        \alvarMarkText "(apercevant Zima)"
      }
    >>
    fa8 fa16 sol la8 si |
