...
\tag #'(zima basse) {
  C’est l’A -- mant que mon cœur vous pré -- fe -- re.
}
\tag #'(alvar basse) {
  O -- sez- vous pro -- non -- cer un Ar -- rêt si fa -- tal.
}
\tag #'(zima basse) {
  Dons nos Fo -- rest on est sin -- ce -- re.
}
\tag #'(alvar basse) {
  Je sçau -- ray m’im -- mo -- ler un o -- di -- eux Ri -- val.
}
\tag #'(adario basse) {
...
\tag #'(damon basse) {
  Ve -- nez ve -- nez plû -- tôt l’a -- mu -- ser à la Fê -- te.
}
