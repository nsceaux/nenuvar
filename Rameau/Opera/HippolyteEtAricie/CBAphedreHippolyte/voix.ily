<<
  ...
  \tag #'(phedre basse) {
    ...
    << \tag #'basse { s2.*2 s1 s2.*2 s1 s4 } \tag #'phedre { r4 r2 R2. R1 R2.*2 R1 r4 } >>
    ...
    mi'4
    << \tag #'basse { s2.*2 s1*4 s2. s2 } \tag #'phedre { r4 r2 R2. R1*4 R2. r2 } >>
    \tag #'basse \phedreMark r8 mi'' mi'' mi'' |
    dod''2\trill r4 la' |
    ...
    sol'2. sol'8 sol' |
    mi'4\trill mi'8 r
    << \tag #'basse { s2 s1*3 s2. s1*12 s4 } \tag #'phedre { r2 R1*3 R2. R1*12 r4 } >>
    \tag #'basse \phedreMark \slurCirc sol''\( do''8.\) do''16 sol'8 la' |
    sib'4. sib'16 re'' sib'4.\arcTrill sib'16 la' |
    la'8\trill la' r do'' \slurCirc fa''4\( fa''16\) re'' re'' re'' |
    ...
    fa''2 la'4 si'8 do'' |
    si'2\trill sol'' ~|
    \slurCirc sol''4\( sol''8\) r r4 r8 mi'' |
  ...
  \tag #'(hippolyte basse) {
    ...
    fad'4
    << \tag #'basse { s2. s1*3 s2. s1 s8 } \tag #'hippolyte { r4 r2 R1*3 R2. R1 r8 } >>
    ...
    fad'4.\trill
    << \tag #'basse { s8 s2 s2. s1*3 s4 } \tag #'hippolyte { r8 r2 R2. R1*3 r4 } >>
    ...
    sold'2\trill
    << \tag #'basse { s2 s1 s2. s1 s2. s4 } \tag #'hippolyte { r2 R1 R2. R1 R2. r4 } >>
    ...
    re'2 re'4 mi'8 fa' |
    mi'4\trill
    << \tag #'basse { s2. s1*10 s2. } \tag #'hippolyte { r4 r2 R1*10 r2 r4 } >>
    \tag #'basse \hippolyteMark re'4 |
    fa'4.
    << \tag #'basse { s8 s2 s1*2 s2. s1*6 s2 s8 } \tag #'hippolyte { r8 r2 R1*2 R2. R1*6 r2 r8 } >>
