<<
  %% Tisiphone
  \tag #'(tisiphone basse) {
    <<
      \tag #'basse { s1*3 s4.*37 }
      \tag #'tisiphone {
        \clef "vbasse" R1*3
        mi'4.^\markup\character Tisiphone |
        re'8 dod' si |
        la sold fad |
        mi la dod' |
        si si mi' |
        dod'\trill la dod' |
        si si mi' |
        dod'4\trill \appoggiatura si8 la |
        r4 dod'16 re' |
        mi'4. ~|
        mi'8 re'16 dod' si la |
        sold4\trill sold8 |
        dod'16 re' dod' si la sold |
        fad8\trill fad si |
        si si mi' |
        fad4.\trill |
        fad8 fad16 fad fad si |
        mi4. |
        mi8 la la |
        la4 la8 |
        la4 la16 la |
        la4. |
        r8 si si |
        si4 si8 |
        si4 si16 si |
        si4 dod'8 |
        re'16 mi' re' dod' si re' |
        dod'8\trill la dod' |
        mi'4. |
        mi'8 re'16 dod' si la |
        si8 mi si |
        mi'4. |
        mi'8 re'16 dod' si la |
        mi4. ~|
        mi |
        la |
        R4. |
      }
    >>
    << \tag #'basse { s1 s2. s1*5 s2. s1*6 s2 }
      \tag #'tisiphone { R1 R2. R1*5 R2. R1*6 r2 } >>
    \tag #'basse \tisiphoneMark re'2-\tag #'tisiphone ^\markup\character Tisiphone |
    r8 la16 la fad4\trill fad8 sol16 la |
    \appoggiatura la8 si4 -\prall si16 r sol sol re8 re16 mi |
    \appoggiatura mi8 fad4 fad8 fad16 fad sol8 la |
    mi4 r16 dod' dod' dod' lad8.\trill dod'16 |
    fad4 dod'8 dod'16 dod'
    << { \voiceOne fad'8. re'16 }
      \new Voice { \voiceTwo \set Voice . autoBeaming = ##f dod'8. dod'16 } >> |
    \oneVoice \appoggiatura dod'8 re'4 re'8 r red'4 red'8 r |
    mi'4. mi'8 r16 la la la la8. sold16 |
    sold1\trill |
    \tag #'tisiphone {
      R1*2 R2. R1*2 R2.*2
      R1*4 |
      r2 sol' ~|
      sol'4 fa'8 mi' re'\trill re' re' mi' |
      \appoggiatura re' do'1 |
      R1*4 |
      r4 r8 mi fad sold la si |
      do'4 do'8 do' la4\trill la8 fad |
      si si dod' re' dod' dod' red' mi' |
      red'4\trill red'8 r mi'4 si8 re' |
      do' do' la fad si2 |
      mi r8 mi mi la |
      sold2\trill sold4 la8 si |
      \appoggiatura si do'4 \appoggiatura si8 la4 r2 |
      r re' ~|
      re'4 mi'8 re' do' si la sol |
      fa2 fa' ~|
      fa'4 si mi'2 ~|
      mi'4 fa'8 mi' re' do' si do' |
      la1\trill |
    }
  }
  %% Thésée
  \tag #'(thesee basse) {
    \tag #'basse \theseeMark \tag #'thesee \clef "vbasse" r4 la8 la dod'4 dod'8 la |
    mi4 si8 dod' re'4 re'8 dod' |
    dod'2\trill dod'4 r |
    << \tag #'basse R4.*37 \tag #'thesee R4.*37 >>
    \tag #'basse \theseeMark
    dod'2-\tag #'thesee ^\markup\character Thésée r8 dod'16 dod' dod'8 re' |
    re'8. fad'16 la4 la8 la16 si |
    \appoggiatura si8 dod'4. r16 dod sold4 sold8 sold16 la |
    la4 la8 dod' la4\trill la8 la |
    fad2\trill fad4 r8 re' |
    re'4. la8 fad4. re8 |
    la,4. la8 lad8.\trill lad16 lad8 si |
    si4. si16 si sold8\trill la16 si |
    \appoggiatura si8 dod'4 la8 la fad4\trill fad8 si |
    mi2 mi8 r mi fad |
    sol4. sol8 dod4. dod16 re |
    re4. fad16 fad si8. re'16 sold8\trill sold16 la |
    mi1 |
    mi'4 ~ mi'16 dod' la fad si,4. si,16 mi |
    la,2 
    << \tag #'basse { s2 s2.*5 s1*3 } \tag #'thesee { r2 R2.*5 R1*3 } >>
    \tag #'basse \theseeMark do'4.-\tag #'thesee ^\markup\character Thésée re'16 mi' si4 ~ si16 sold la si |
    \appoggiatura si8 do'4 \appoggiatura si8 la4 r r8 fa |
    fa4. fa16 mi la8 la16 re |
    \appoggiatura re8 mi4.\prall r16 si si4 si8 dod'16 re' |
    dod'4\trill mi'8 la la8.([ sol16\trill)] fa8 mi |
    \appoggiatura mi fa4 fa16 r la la re'8 re'16 fa' |
    si4 do'8 la16 la fa8 sol |
    do2 r8 mi mi la |
    sold2\trill sold4 la8 si |
    \appoggiatura si do'4 \appoggiatura si8 la r
    \tag #'thesee {
      do'2 ~|
      do'4 re'8 do' si la sol fa |
      mi2 mi' ~|
      mi'4 re'8 do' fa fa fa sol |
      do1 |
      r4 r8 do' si la sol fa |
      mi4 re8 do sol4 sol8 do' |
      si2\trill re'4 re'8 mi' |
      fa'2 ~ fa'8 do' do' re' |
      \appoggiatura re' mi'2 mi'8 r r4 |
      R1*2 |
      r2 r8 sol la si |
      fad2 fad4 sold8 la |
      sold2\trill sold8 r r4 |
      R1 |
      r2 do' ~|
      do'4 re'8 do' si la sol fa |
      mi2 mi' ~|
      mi'4 fa'8 mi' re' do' si la |
      sold2.\trill la8 do |
      re2 re4 re8 mi |
      la,1 |
    }
  }
>>